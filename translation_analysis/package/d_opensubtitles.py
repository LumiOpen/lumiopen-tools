...
# Globals
DATA_PATH = "../../data"
FI_DATASET_PATH = f"{DATA_PATH}/opensubtitles-2016_en-fi/OpenSubtitles.en-fi.fi"
EN_DATASET_PATH = f"{DATA_PATH}/opensubtitles-2016_en-fi/OpenSubtitles.en-fi.en"


def main(per: int, bands: int, thold: float, minlen: int):
    """
    Handler script for OpenSubtitles-2016.
    ...
    if not os.path.isfile(FI_DATASET_PATH) and os.path.isfile(EN_DATASET_PATH):
        logging.error("OpenSubtitles-2016 dataset files could not be found. Program will now exit.")
        sys.exit(1)
    ...
    with open(f"{DATA_PATH}/out/opensubtitles-2016_sampled_entries.json", mode='w') as file:
